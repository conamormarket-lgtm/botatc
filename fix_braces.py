...
# First let's put the block I have, and replace braces with double braces!
# Wait, I already overwrote server.py with the BAD unescaped braces!
# I can just re-rewrite the entire block in server.py, this time completely escaped!

bad_func_block = text[start_idx:end_idx]

raw_js = r"""            function renderQuickReplies(data) {
...
escaped_js = raw_js.replace("{", "{{").replace("}", "}}")

new_text = text[:start_idx] + escaped_js + "\n" + text[end_idx:]

with open("server.py", "w", encoding="utf-8") as f:
    f.write(new_text)

print("Replaced with perfectly escaped JS!")
