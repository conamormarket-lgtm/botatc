...
target = r"""    // Función principal para enviar texto plano
    window.enviarMensajeManual = async function (wa_id) {"""
...
injection = r"""
    ...
    window.uploadAdminMedia = async function(inputElem, wa_id) {
        ...
        try {
            ...
            if(!data.ok){
                ...
                fetchInbox(); // Refrescar para ver el archivo guardado
            }
        } catch (e) {
    ...
    // Agregar soporte para Pegar (Paste) en el input
    document.addEventListener('paste', async function(e) {
        ...
        for (let index in items) {
            ...
            if (item.kind === 'file') {
                ...
                if (confirm(`¿Pegar y enviar esta imagen al chat de ${wa_id}?`)) {
                    // Simular subida usando el mismo flujo
    ...
    // Función principal para enviar texto plano
    window.enviarMensajeManual = async function (wa_id) {"""
...
if target in text:
    ...
    with open("inbox.html", "w", encoding="utf-8") as f:
        f.write(text)
    print("Injected inbox.html logic successfully.")
else:
