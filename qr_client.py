...
def _post(endpoint: str, payload: dict) -> dict | None:
    ...
    try:
        ...
        body = e.read().decode()
        print(f"[QR_CLIENT] ❌ HTTP {e.code} en {endpoint}: {body}")
        return None
    except Exception as e:
        print(f"[QR_CLIENT] ❌ Error en POST {endpoint}: {e}")
        return None
...
def _get(endpoint: str) -> dict | None:
    ...
    try:
        ...
        print(f"[QR_CLIENT] ❌ Error en GET {endpoint}: {e}")
        return None
...
def _get_bytes(endpoint: str) -> tuple[bytes | None, str | None]:
    ...
    try:
        ...
        print(f"[QR_CLIENT] ❌ Error descargando bytes de {endpoint}: {e}")
        return None, None
...
async def enviar_reaccion_qr(
    ...
    if result and result.get("status") == "ok":
        print(f"[QR_CLIENT] ✅ Reacción '{emoji}' enviada a {numero_destino}")
        return True
    print(f"[QR_CLIENT] ❌ Fallo enviando reacción a {numero_destino}: {result}")
    return False
...
def resolver_lid_a_telefono(lid: str, line_id: str = "qr_ventas_1") -> str | None:
    ...
    if result and result.get("phone"):
        phone = result["phone"]
        print(f"[QR_CLIENT] ✅ LID {lid} resuelto → {phone}")
        return phone
    print(f"[QR_CLIENT] ⚠️ No se pudo resolver LID {lid}")
    return None
